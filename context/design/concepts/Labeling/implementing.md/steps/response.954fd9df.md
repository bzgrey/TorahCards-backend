---
timestamp: 'Sun Oct 12 2025 19:47:25 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251012_194725.f4c08b67.md]]'
content_id: 954fd9df6e808c9e31af4fc55e47cd028bddac2bc83a2fb111bfa54b022b0120
---
...
import { Empty, ID } from "../../utils/types.ts"; // Adjust path as needed based on your project structure
...
interface GetLabelItemsResult {
  items: Item[];
}

interface GetItemLabelsResult {
  labels: Label[];
}
...
export default class LabelingConcept {
  ...
  async deleteItem({ item }: { item: Item }): Promise<Empty | ErrorResult> {
    ...
    if (itemDoc.labels && itemDoc.labels.length > 0) {
      await this.labelsCollection.updateMany(
        { _id: { $in: itemDoc.labels } }, // Find all labels associated with this item
   ...
   * @returns {Promise<GetLabelItemsResult | ErrorResult>} An object containing an array of Item IDs on success, or an error object if the label is not found.
   *
  ...
  async _getLabelItems({
    ...
    labelName: string;
  }): Promise<GetLabelItemsResult | ErrorResult> {
    const labelDoc = await this.labelsCollection.findOne({ name: labelName });
    if (!labelDoc) {
      return { error: `Label '${labelName}' not found.` };
    }
    return { items: labelDoc.items || [] };
  }

  /**
   * @query _getItemLabels
   * Retrieves the set of labels associated with a given item.
   *
   * @param {Item} item - The ID of the item.
   * @returns {Promise<GetItemLabelsResult | ErrorResult>} An object containing an array of Label IDs on success, or an error object if the item is not found.
   *
   * @requires given item exists.
   * @effects returns set of Labels (IDs) associated with given item.
   */
  async _getItemLabels({
    item,
  }: {
    item: Item;
  }): Promise<GetItemLabelsResult | ErrorResult> {
    const itemDoc = await this.itemsCollection.findOne({ _id: item });
    if (!itemDoc) {
      return { error: `Item '${item}' not found.` };
    }
    return { labels: itemDoc.labels || [] };
  }
}
```
