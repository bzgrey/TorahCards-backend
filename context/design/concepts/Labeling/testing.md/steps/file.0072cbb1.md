...
content_id: 0072cbb196b77397e9292819dc5660fc48c8b6723fbfd23be637d90311730110
---
...
interface ItemDoc {
  _id: Item;
  labels: string[]; // Array of label names associated with this item
}
...
export default class LabelingConcept {
  ...
  async addLabel({
    ...
    if (itemDoc && itemDoc.labels.includes(labelDoc.name)) {
  ...
  async deleteLabel({
    ...
    if (!itemDoc.labels.includes(labelDoc.name)) {
  ...
  async deleteItem({ item }: { item: Item }): Promise<Empty | ErrorResult> {
    ...
    if (itemDoc.labels && itemDoc.labels.length > 0) {
      await this.labelsCollection.updateMany(
        { name: { $in: itemDoc.labels } }, // Find all labels associated with this item
   ...
   *
   * @param {Item} item - The ID of the item.
   * @returns {Promise<string[] | ErrorResult>} Array of the label names, or an error object if the item is not found.
   *
