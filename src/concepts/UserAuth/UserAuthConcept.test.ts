...
Deno.test("UserAuth Concept Tests", async (test) => {
  ...
  await test.step("testPrinciple: User registration, login, and authentication", async () => {
    ...
    const aliceId = (registerResult as { user: ID; token: ID }).user;
    const aliceRegisterToken =
      (registerResult as { user: ID; token: ID }).token;
    console.log(
      `User 'alice' registered successfully with ID: ${aliceId}, token: ${aliceRegisterToken}`,
    );
    ...
    const aliceLoginToken = (loginResult as { token: ID }).token;
    console.log(
      `User 'alice' logged in successfully with new token: ${aliceLoginToken}`,
    );
    assertEquals(
      aliceRegisterToken !== aliceLoginToken,
      true,
    ...
    console.log(
      "Attempting to get authenticated user for the new login token...",
    );
    const authenticatedUserResult = await userAuth._getAuthenticatedUser({
    ...
    assertEquals(authenticatedUserResult.length, 1);
    assertEquals(authenticatedUserResult[0].user, aliceId);
    console.log(
      `Authenticated user for token '${aliceLoginToken}' is: ${
        authenticatedUserResult[0].user
      }`,
    );

    console.log(
      "Operational principle successfully demonstrated: Register -> Login -> Authenticate with new token.",
    );
  ...
  await test.step("Scenario 1: register - Username uniqueness", async () => {
    ...
    const bobRegisterResult1 = await userAuth.register({
      username: "bob",
      password: "bob_pass",
    });
    ...
    console.log(
      "Attempting to register user 'bob' again with the same username...",
    );
    const bobRegisterResult2 = await userAuth.register({
      username: "bob",
    ...
    console.log(
      `Attempt to register 'bob' again failed as expected: ${
        (bobRegisterResult2 as { error: string }).error
      }`,
    );
  ...
  await test.step("Scenario 2: login - Invalid credentials", async () => {
    ...
    const carolLoginWrongPass = await userAuth.login({
      username: "carol",
      password: "wrong_pass",
    });
    assertEquals("error" in carolLoginWrongPass, true);
    console.log(
      `Login attempt for 'carol' with wrong password failed as expected: ${
        (carolLoginWrongPass as { error: string }).error
      }`,
    );

    // Attempt to login a non-existent user
    console.log("Attempting to log in non-existent user 'dave'...");
    const daveLoginResult = await userAuth.login({
      username: "dave",
      password: "dave_pass",
    });
    assertEquals("error" in daveLoginResult, true);
    console.log(
      `Login attempt for 'dave' failed as expected: ${
        (daveLoginResult as { error: string }).error
      }`,
    );
  ...
  await test.step("Scenario 3: logout - Token invalidation", async () => {
    ...
    const eveRegisterResult = await userAuth.register({
      username: "eve",
      password: "eve_pass",
    });
    const eveToken = (eveRegisterResult as { user: ID; token: ID }).token;
    ...
    console.log(
      `Attempting to authenticate with invalidated token: ${eveToken}`,
    );
    const authWithInvalidToken = await userAuth._getAuthenticatedUser({
      token: eveToken,
    });
    assertEquals("error" in authWithInvalidToken, true);
    console.log(
      `Authentication with invalidated token failed as expected: ${
        (authWithInvalidToken as { error: string }).error
      }`,
    );
  ...
  await test.step("Scenario 4: Query _getUsername and _getPassword", async () => {
    ...
    const frankRegisterResult = await userAuth.register({
      username: "frank",
      password: "frank_pass",
    });
    ...
    const graceRegisterResult = await userAuth.register({
      username: "grace",
      password: "grace_pass",
    });
    ...
    let frankPasswordResult = await userAuth._getPassword({ user: frankId });
    assertEquals(Array.isArray(frankPasswordResult), true);
    frankPasswordResult = frankPasswordResult as { password: string }[];
    assertEquals(frankPasswordResult.length, 1);
    assertEquals(
      frankPasswordResult[0].password,
      "frank_pass",
    );
    ...
    const multipleUsernamesResult = await userAuth._getUsername({
      users: [frankId, graceId],
    });
    ...
    const sortedUsernames = (multipleUsernamesResult as { username: string }[])
      .sort((a, b) => a.username.localeCompare(b.username));
    assertEquals(sortedUsernames[0].username, "frank");
    assertEquals(sortedUsernames[1].username, "grace");
    console.log(
      `Usernames found: ${sortedUsernames.map((u) => u.username).join(", ")}`,
    );
    ...
    const nonExistentUsernameResult = await userAuth._getUsername({
      users: [nonExistentId],
    });
    assertEquals("error" in nonExistentUsernameResult, true);
    console.log(
      `Query for non-existent user failed as expected: ${
        (nonExistentUsernameResult as { error: string }).error
      }`,
    );
